...
def solve(text,evaluation_text):
  ...
  s1 = nlp1(input_answers)
  s2 = nlp1(correct_answers)
  ...
  pre = (verb_score+noun_score+adj_score+pro_score+adp_score+sym_score+num_score) / (len(s1_verbs)+len(s1_nouns)+len(s1_adj)+len(s1_pronoun)+len(s1_adp)+len(s1_sym)+len(s1_num))
